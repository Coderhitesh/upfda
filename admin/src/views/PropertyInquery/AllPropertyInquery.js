...
const AllPropertyInquiry = () => {
    ...
    const fetchPropertyInquiries = async () => {
        setLoading(true);
        try {
            const { data } = await axios.get('https://www.test.blueaceindia.com/api/v1/get_property_inquery');
    ...
    const handleDeleteInquiry = async (id) => {
        try {
            setLoading(true);
            await axios.delete(`https://www.test.blueaceindia.com/api/v1/delete_property_inquery/${id}`);
